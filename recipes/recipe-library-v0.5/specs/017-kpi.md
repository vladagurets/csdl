...
- `Pulse directs Axis` is forbidden because it substitutes another semantic mechanism.

## Assembly order

1. place primary Pulse.
2. attach period and label.
3. show restrained supporting measures.
...
- Required bindings: `headline`, `primary`, `supporting`, `source_label`.
...
- Semantic mechanism: `kpi-mechanism`.
...
- Component cardinality: `Pulse`=1, `Axis`=1, `Label`=5, `Signal`=1.
- Expression: `A`.
- Density: `analytical`.
...
- `patterns/visual-dna-sprint-01/manifest.yaml` — Problem, scenarios, components, assembly, levels, content, and exclusions. (family 17 KPI).
- `patterns/visual-dna-sprint-01/specs/17-kpi.md` — Canonical Markdown semantic and Prompt DSL evidence. (complete family contract).
- `patterns/visual-dna-sprint-01/canonical/light/16x9/17-kpi.png` — Accepted presentation-scale example. (canonical raster).
