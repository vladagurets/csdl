...
- Added the repository-local `csdl-create` skill under `ai/skills/` with Codex and Claude symlinks, a mandatory brief/clarification approval gate, canonical CSDL generation and selection workflow, pilot output contract, UI metadata, and a tested variable-length validator for contiguous slides, three candidates per slide, user-approved A/B/C rhythm, exact-copy prompts, canonical rasters, previews, contact sheet, review hashes, and rubric scores; the seven-slide rhythm remains a default rather than a hard limit.
- Added an additive seven-slide English demo infographic under `docs/demo-infographic/`: translated the original five-slide narrative, added Recipes and human/agent contract bridges, generated and reviewed three candidates per slide, selected seven `1920×1080` RGB PNG files, and assembled a `3840×2160` contact sheet without changing accepted raster evidence or beginning Milestone 8.
...
- Added strict publication validation, twelve exact-error negative fixtures, focused two-build tests, CI rebuild/validation/drift checks, and persisted full-page/contact-sheet/text-layer/grayscale review evidence.
- Completed Milestone 7 — Cookbook and Design Book v1.0 through green integration PR #69 and merge commit `4c20829f4923c164b48985d06a49247ff372ed4f`; all publication, dependency, deterministic-build, and sixty-raster hash gates pass without beginning Milestone 8 or making a release/licensing decision.
...
- Added Milestone 6 CI rebuild/validation/drift gates, a deterministic SHA-256 inventory for all sixty tracked accepted PNGs, library documentation, and completion alignment across README, STATUS, ROADMAP, AGENTS, and the changelog.
...
- Added ten deterministic end-to-end analytical proofs, indexed positive fixtures, seventeen exact-error negative mutations, strict deterministic rebuild/index checks, synthetic-proof review evidence, and additive migration/rollback documentation without raster generation.
...
- Added vocabulary-only editorial Big Number, structural Architecture, and fixed-data analytical Chart composition proofs; generated the deterministic component index and 15×20 compatibility matrix, with negative tests for ad hoc layout primitives, forbidden relations, and quantitative distortion.
