# Pilot 02 — Superpowers vs Compound Engineering

Eight-slide Ukrainian CSDL presentation series comparing two process-oriented approaches to disciplined AI-assisted software development.

## Approved sequence

`A → A → B → A → B → A → C → A`

## Output contract

- eight canonical `1920×1080` RGB PNG slides;
- three reviewed built-in GPT Image 2 candidates per slide;
- eight `1280×720` landscape previews;
- one `3840×2160` contact sheet;
- Prompt DSL v0.5 packages, source map, scores, and selection evidence;
- no change to Pilot 01 or any previously accepted raster.

## Source of truth

Visible copy, Recipe choice, components, mechanisms, and expression levels live in `manifest.yaml`. No raster may contain text absent from the matching `exact_copy` list.

## Status

Complete. Twenty-four normalized candidates were reviewed, eight canonical slides were promoted, all exact-copy and rubric gates pass, and the final preview/contact-sheet package is available under `previews/landscape/` and `contact-sheets/pilot-02-light.png`.
