...
Pilot 02 is complete under `pilots/02-superpowers-vs-compound-engineering/`. The additive eight-slide Ukrainian series compares Superpowers and Compound Engineering as orthogonal disciplines for within-task execution and between-task knowledge capture; its central comparison uses the user-approved three-row scope of value, emphasis, and risk, its culmination uses the standard Ukrainian term `інструмент`, and its closing slide now gives current installation paths for both Codex and Claude Code. It includes 24 reviewed GPT Image 2 candidates, eight exact-copy `1920×1080` RGB canonicals, eight `1280×720` previews, one centered `3840×2160` contact sheet, current primary-source fact mapping, Prompt DSL v0.5 packages, hashes, selection evidence, and passing rubric scores. The work does not alter Pilot 01, any previously accepted raster, Prompt DSL v0.5, the fifteen-component vocabulary, the 23-recipe library, or the completed Milestone 7 publication contract.
...
accessibility/night-mode-v0.1/manifest.yaml
accessibility/night-mode-v0.1/SPEC.md
accessibility/night-mode-v0.1/token-schema.yaml
accessibility/night-mode-v0.1/proof-schema.yaml
accessibility/night-mode-v0.1/contracts/tokens.yaml
accessibility/night-mode-v0.1/contracts/contrast.yaml
accessibility/night-mode-v0.1/contracts/fallbacks.yaml
accessibility/night-mode-v0.1/contracts/compatibility.yaml
accessibility/night-mode-v0.1/proofs/packages/01-editorial-equivalence.yaml through 10-projector-fallback.yaml
accessibility/night-mode-v0.1/index.yaml
...
accessibility mode built: 10 proof packages, index.yaml, contrast-matrix.yaml, compatibility.yaml, raster-hashes.yaml
